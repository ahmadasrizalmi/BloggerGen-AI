...
import { ArticleParams, GenerationStatus, ToastMessage } from './types';
...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col">
      ...
      {/* Toast */}
      {toast && (
        <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-[60] px-6 py-3 rounded-full shadow-xl flex items-center gap-3 text-sm font-medium animate-fade-in-up ${
                   ...
                   {generatedHtml && (
                     <>
                       <button 
                        ...
                        className="px-4 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-xs font-bold hover:bg-slate-50 hover:border-slate-400 flex items-center gap-2 transition-all active:scale-95"
                       >
                         <Copy size={14}/> 
                         Copy Visual
                       </button>
                       <button 
                        ...
                        className="px-4 py-2 bg-slate-800 text-white rounded-lg text-xs font-bold hover:bg-slate-700 flex items-center gap-2 transition-all shadow-lg shadow-slate-900/10 active:scale-95"
                       >
                         <Code size={14}/> 
                         Copy HTML
                       </button>
              ...
              <div className="flex-grow relative p-0">
                {status === GenerationStatus.IDLE && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
                    ...
                    <p className="text-sm mt-2 text-slate-500">Add affiliate links in the sidebar to monetize your content automatically.</p>
                  </div>
                )}
                ...
                {status === GenerationStatus.SUCCESS && generatedHtml && (
                  <div className="h-full overflow-y-auto custom-scrollbar p-8 bg-white">
                    {activeTab === 'preview' ? (
                      ...
                      <pre className="text-xs font-mono bg-slate-900 text-green-400 p-6 rounded-xl overflow-x-auto whitespace-pre-wrap break-all">
                        {generatedHtml}
                      </pre>
                    )}
                  </div>
